...
const loadTextAndToggleJsdoc =
  ...
  async () => {
    const targetContent = await loadFile(resultsFolder, fileName)
...
const itHasCorrectOutputAndSelectionPositions =
  ...
  () => {
    ...
    itHasTargetText(resultsFolder, targets, fileName, initialRange)
    itHasCursorSelectionPosition(resultsFolder, targets, fileName, initialRange)
  }
...
const itHasCorrectOutputAndCursorPosition =
  ...
  (
    ...
    itHasCorrectOutputAndSelectionPositions(resultsFolder)(
      fileName,
      cursorInitialLine,
      cursorInitialChar,
      cursorInitialLine,
...
const runTestSuite = (resultsFolder: string) => () => {
  // convenience wrappers that bind `resultsFolder` via currying
  const C = (
    ...args: Parameters<ReturnType<typeof itHasCorrectOutputAndCursorPosition>>
  ) => itHasCorrectOutputAndCursorPosition(resultsFolder)(...args)
  const S = (
    ...args: Parameters<
      ReturnType<typeof itHasCorrectOutputAndSelectionPositions>
    >
  ) => itHasCorrectOutputAndSelectionPositions(resultsFolder)(...args)
  ...
  describe("single line jsdoc comment", () => {
    ...
    describe("adding new comment", () => {
      ...
      describe.skip(
        ...
        S("singleAddSelection.js", 0, 0, 1, 0, 0, 4, -1, 26),
      )

      // TODO: improve weak example
      describe(
        "when selection exists within interior of line",
        S("singleAddSelectionInternal.js", 0, 21, 0, 32, 0, 4, 0, 4),
      )

      // TODO: improve weak example
      describe(
        "when selection ends at end of line",
        S("singleAddSelectionEnd.js", 0, 19, 0, 24, 0, 4, 0, 4),
      )
      ...
      describe.skip(
        ...
        S("singleAddSelectionIndent.js", 1, 0, 2, 0, 0, 6, -1, 28),
      )
      ...
      describe.skip(
        ...
        S("singleAddSelectionIndentAlone.js", 2, 0, 3, 0, 0, 6, -1, 28),
      )
      describe(
        "when selection is all of the text and indented",
        S("singleAddSelectionIndent.js", 1, 2, 1, 24, 0, 4, 0, 4),
      )
      describe(
        "when reversed selection is all of the text and indented",
        S("singleAddSelectionIndent.js", 1, 24, 1, 2, 0, 4, 0, 4),
      )
    })
    ...
    describe("when existing comment", () => {
      describe("when it's a line comment", () => {
        ...
        describe("when it's alone on a line", () => {
          ...
          describe("when selection", () => {
            describe.skip(
              ...
              S("singleConvertLine.js", 2, 0, 3, 0, 0, 0, 0, 0),
            )
          })
        })
        ...
        describe("when it's at start of a line", () => {
          describe("when NO selection", () => {
            describe(
              "when cursor's inside",
              C("singleConvertLineStart.js", 0, 10, 0, 1),
            )

            describe(
              "when cursor is just before comment tag",
              C("singleConvertLineStart.js", 0, 0, 0, 3),
            )

            describe(
              "when cursor is inside comment tag",
              C("singleConvertLineStart.js", 0, 1, 0, 2),
            )

            describe(
              "when cursor is just after comment tag before a space",
              C("singleConvertLineStart.js", 0, 2, 0, 1),
            )

            describe(
              "when cursor is just after space after comment tag",
              C("singleConvertLineStart.js", 0, 3, 0, 1),
            )

            describe(
              "when cursor is at end of line comment",
              C("singleConvertLineStart.js", 0, 25, 0, 1),
            )
          })
          // TODO: implement
          // describe("when selection", () => {
        ...
        describe("when it's nested in JSDoc", () => {
          ...
          describe(
            ...
            C("singleConvertLineNestedA.js", 1, 21, 0, -1),
          )
          // TODO: can be improved by also aligning the comment
          describe(
            "when NO star proceeds it in middle",
            C("singleConvertLineNestedB.js", 2, 21, 0, -1),
          )
          // FIXME: rm extra space
          describe(
            "when a star proceeds it",
            C("singleConvertLineNestedC.js", 2, 21, 0, -2),
          )
        })
        ...
        describe("when it's trailing code", () => {
          describe("when NO selection", () => {
            describe(
              "when cursor is before comment",
              C("singleConvertLineTrailingB.js", 2, 6, 0, 0),
            )
            describe(
              "when cursor's inside",
              C("singleConvertLineTrailing.js", 2, 25, 0, -12),
            )
            ...
            describe.skip(
              "when cursor is just before comment tag",
              C("singleConvertLineTrailing.js", 2, 15, 0, -9),
            )
            // FIXME: doesn't get rid of comment tag
            describe.skip(
              "when cursor is inside comment tag",
              C("singleConvertLineTrailing.js", 2, 16, 0, -10),
            )
            // FIXME: adds an extra space between jsdoc start and comment
            describe.skip(
              "when cursor is just after comment tag before a space",
              C("singleConvertLineTrailing.js", 2, 17, 0, -11),
            )
            describe(
              "when cursor is just after space after comment tag",
              C("singleConvertLineTrailing.js", 2, 18, 0, -12),
            )
            describe(
              "when cursor is at end of line comment",
              C("singleConvertLineTrailing.js", 2, 40, 0, -12),
            )
          })
          // TODO: implement
          // describe("when selection", () => {
      ...
      describe("when it's a block comment", () => {
        describe(
          ...
          C("singleConvertBlock.js", 1, 12, 0, 1),
        )
        describe(
          "when it's alone on a line & cursor before it",
          C("singleConvertBlock.js", 1, 0, 0, 0),
        )
        describe(
          "when it's alone on a line & cursor after it",
          C("singleConvertBlock.js", 1, 30, 0, 1),
        )

        describe(
          "when it's internal & surrounded by code",
          C("singleConvertBlockInternal.js", 1, 23, 0, 1),
        )
        describe(
          "when it's leading code",
          C("singleConvertBlockLeading.js", 2, 18, 0, 1),
        )
        describe(
          "when it's trailing comment",
          C("singleConvertBlockTrailing.js", 3, 23, 0, -12),
        )
        describe(
          "when it's trailing comment inside comma",
          C("singleConvertBlockTrailingComma.js", 2, 23, 0, -10),
        )
        // FIXME: doesn't add space afterward
        describe.skip(
          "when it's missing spaces and ugly",
          C("singleConvertBlockUnspaced.js", 1, 11, 0, 2),
        )
      })
    })
    ...
    describe("remove existing jsdoc", () => {
      describe("when it's alone on a line", () => {
        describe(
          "when cursor is before comment",
          C("singleRemove.js", 1, 0, 0, 0),
        )

        describe("when cursor's inside", C("singleRemove.js", 1, 20, 0, -1))
        describe("when cursor's at end", C("singleRemove.js", 1, 31, 0, -4))
      })
      describe(
        "when it's internal & surrounded by code",
        C("singleRemoveInternal.js", 1, 24, 0, -1),
      )
      describe(
        "when it's leading code",
        C("singleRemoveLeading.js", 2, 13, 0, -1),
      )
      describe(
        "when it's trailing code",
        C("singleRemoveTrailing.js", 1, 25, 0, -1),
      )
      describe("when it's unspaced", C("singleRemoveUnspaced.js", 1, 12, 0, 0))
      describe(
        "when the whole line is selected from end to start",
        S("singleRemoveUnspaced.js", 1, 29, 1, 0, 0, -2, 0, 0),
      )
    })
    ...
    describe("multi line jsdoc comment", () => {
      describe("add", () => {
        ...
        describe(
          ...
          S(
            ...
            1,
            2,
            5, // TODO: consider not including JSDoc opening `/**` in selection and make next two params 1, 3
            0,
            0,
            1,
            3,
        ...
        describe(
          "selection is at end",
          S("multiAdd.js", 2, 13, 1, 5, 1, 3, 1, 3),
        )
      })
      ...
      describe("convert", () => {
        describe("when it's a line comment", () => {
          describe(
            "selection not before or at end (internal)",
            S("multiConvertLine.js", 2, 9, 1, 9, 1, 1, 1, 0),
          )
          ...
          describe(
            ...
            S("multiConvertLine.js", 2, 15, 1, 9, 1, 1, 1, 0),
          )
          ...
          describe("when it's nested in JSDoc", () => {
            ...
            describe.skip(
              ...
              S("multiConvertLineNestedA.js", 2, 30, 3, 30, 0, -2, 0, -1),
            )
          })
        })
      })
...
describe("default behavior", runTestSuite("../../../test/results/"))

describe("when `toggleBetweenNoComment` is true", () => {
  before(async () => {
    const config = vscode.workspace.getConfiguration("jsdoc-comment-toggler")
    await config.update("toggleBetweenNoComment", true, true)
  })

  describe("tests", runTestSuite("../../../test/results-toggle/"))

  after(async () => {
    const config = vscode.workspace.getConfiguration("jsdoc-comment-toggler")
    await config.update("toggleBetweenNoComment", undefined, true)
  })
})
